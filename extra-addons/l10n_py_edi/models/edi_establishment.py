...
class EDIEstablishment(models.Model):
    _name = "edi.establishment"
    ...
    issuance_point_ids = fields.One2many('edi.establishment.issue.point', 'parent_id', string="Dispatch Point")
    ...
    def get_dispatch_point(self, **kwargs):
        for dp in sorted(self.issuance_point_ids, key=lambda x: x.sequence):
...
class EDIEstablishmentIssuePoint(models.Model):
    _name = 'edi.establishment.issue.point'
    _description = 'Issue Point'

    parent_id = fields.Many2one('edi.establishment', string="Establishment", ondelete="cascade")
    sequence = fields.Integer('Sequence')
